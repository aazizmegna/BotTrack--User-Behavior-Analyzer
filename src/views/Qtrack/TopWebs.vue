  ...
  <div>
    <v-row class="">
      ...
      {{ clickedItemDuration }}
        ...
        <!-- <p class="mt-10 text-h5">  {{ clickedItemName }}</p> -->
        <v-card 
        class="mt-1"
...
export default {
  ...
  methods: {
    onRowClick(item, index) {
      ...
      console.log('date selectionner',this.dataDate)
    ...
    async loadFirstItem() {
      if (this.categories.length > 0) {
        ...
        this.clickedItemName = firstItem.category;
        this.clickedCatId = firstItem.acrId;
        this.selectedRowIndex = 0;
      this.clickedItemDuration = firstItem.duration

        await this.getSubApp(this.dataDate);
      }
    },
    async getArchival(data) {
      console.log('liste data' , data);
      await getTopWebFilter(
        this.userId,
        data,
